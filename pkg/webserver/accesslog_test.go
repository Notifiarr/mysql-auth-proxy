...
func TestRefererPathForLog(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			origURI: "/api/v1/route/method/" + webserver.TestAccessLogAPIKey,
...
func TestGetAPIKeyFromURIPath(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			pathStr: "/api/v1/route/method/" + webserver.TestAccessLogAPIKey,
			want:    webserver.TestAccessLogAPIKey,
		},
		{
			name:    "strips query from last segment when fused",
			pathStr: "/api/v1/route/method/" + webserver.TestAccessLogAPIKey + "?foo=bar",
			want:    webserver.TestAccessLogAPIKey,
		},
		{
			name:    "empty",
