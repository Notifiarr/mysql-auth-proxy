...
const TestAccessLogAPIKey = "aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee"

// writeLogLine mirrors responseWriter.writeAccessLogLine (prefix + tail) for unit tests.
func writeLogLine(builder *strings.Builder, req *http.Request, writer *captureWriter) {
...
func TestWriteAccessLogLine_MaskedKeyFromResponseHeader(t *testing.T) {
	...
	masked, keyLenStr := maskAPIKey(TestAccessLogAPIKey)
